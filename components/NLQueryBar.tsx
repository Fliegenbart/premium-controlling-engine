    ...
    <div className="mb-6">
      ...
      <form onSubmit={handleSubmit} className="relative">
        <div className="flex items-center bg-white/80 backdrop-blur-2xl rounded-xl border border-black/[0.10] focus-within:border-[#0071e3]/60 focus-within:ring-4 focus-within:ring-[#0071e3]/10 transition-colors shadow-[0_20px_70px_-55px_rgba(0,0,0,0.30)]">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-gray-900 px-4 py-3.5 focus:outline-none placeholder-gray-400 text-sm"
          />
          {query && (
            <button
              type="button"
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {suggestions.map((s, i) => (
            <button
              key={i}
              onClick={() => setQuery(s)}
              className="px-3 py-1.5 bg-white/70 hover:bg-white/90 text-gray-700 hover:text-gray-900 text-xs rounded-lg transition-colors border border-black/[0.08]"
            >
              {s}
            </button>
          ))}
      ...
      {result && (
        <div className="mt-4 bg-white/70 backdrop-blur-2xl rounded-xl border border-black/[0.10] overflow-hidden shadow-[0_30px_100px_-70px_rgba(0,0,0,0.35)]">
          ...
          <div className="flex items-center justify-between p-4 border-b border-black/[0.08]">
            <div className="flex items-center gap-3">
              ...
              <span className={`px-2 py-0.5 rounded text-xs ${
                result.confidence > 0.8 ? 'bg-green-500/20 text-green-700' :
                result.confidence > 0.5 ? 'bg-yellow-500/20 text-yellow-700' :
          ...
          {/* SQL Query (collapsed) */}
          {showSQL && (
            <div className="p-4 bg-white/60 border-b border-black/[0.08]">
              <pre className="text-xs text-gray-700 font-mono overflow-x-auto whitespace-pre-wrap">{result.sql}</pre>
            </div>
          )}
          ...
          {result.result.rows.length > 0 && (
            <div className="overflow-x-auto">
                ...
                <thead className="bg-black/[0.03]">
                  <tr>
                    {result.result.columns.map((col, i) => (
                      <th key={i} className="p-3 text-left text-gray-700 font-semibold">{col}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {result.result.rows.slice(0, 20).map((row, i) => (
                    <tr key={i} className="border-t border-black/[0.06] hover:bg-black/[0.02]">
              ...
              {result.result.rowCount > 20 && (
                <p className="p-3 text-xs text-gray-600 text-center border-t border-black/[0.06]">
